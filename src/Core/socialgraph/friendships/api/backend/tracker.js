...
export default class FriendshipTracker {
  constructor(
    ...
    this.socket.on('inboundFriendshipsChange', inboundChange => {
      console.log(inboundChange)
      if (inboundChange.operationType == 'insert') {
        this.inboundFriendships = this.modifiedFriendshipsAfterAdding(
          this.inboundFriendships,
          inboundChange.fullDocument,
        )
        this.hydrateFriendships()
      } else if (inboundChange.operationType == 'delete') {
        this.inboundFriendships = this.modifiedFriendshipsAfterRemoval(
          this.inboundFriendships,
          inboundChange.documentKey,
        )
        this.hydrateFriendships()
      }
    })
    this.socket.on('outboundFriendshipsChange', outboundChange => {
      console.log(outboundChange)
      if (outboundChange.operationType == 'insert') {
        this.outboundFriendships = this.modifiedFriendshipsAfterAdding(
          this.outboundFriendships,
          outboundChange.fullDocument,
        )
        this.hydrateFriendships()
      } else if (outboundChange.operationType == 'delete') {
        this.outboundFriendships = this.modifiedFriendshipsAfterRemoval(
          this.outboundFriendships,
          outboundChange.documentKey,
        )
        this.hydrateFriendships()
      }
    })
  ...
  subscribeIfNeeded = () => {
    ...
    if (!state.friends.didSubscribeToFriendships) {
      ...
      this.socket.on('connect', () => {
        this.socket.emit('subscriptiontoinbound', {
          watcherID: userId,
          watchedID: userId,
        })
        this.socket.emit('subscriptiontooutbound', {
          watcherID: userId,
          watchedID: userId,
        })
      })
      ...
      this.inboundFriendshipsUnsubscribe =
        friendshipAPI.subscribeToInboundFriendships(
          userId,
          this.onInboundFriendshipsUpdate,
        )

      this.outboundFriendshipsUnsubscribe =
        friendshipAPI.subscribeToOutboundFriendships(
          userId,
  ...
  addFriendRequest = (fromUser, toUser, callback) => {
    ...
    friendshipAPI.addFriendRequest(
      fromUser,
      toUser,
  ...
  cancelFriendRequest = (outBound, toUser, callback) => {
    ...
    friendshipAPI.cancelFriendRequest(
      outBound.id || outBound.userID,
      toUser.id || outBound.userID,
      this.persistFriendshipsCounts,
      this.enableFeedUpdates,
  ...
  modifiedFriendshipsAfterRemoval = (friendships, docToBeRemoved) => {
    return friendships.filter(user => user?.id !== docToBeRemoved?._id)
  }

  modifiedFriendshipsAfterAdding = (friendships, userToBeAdded) => {
    return friendships.concat([userToBeAdded])
  }
